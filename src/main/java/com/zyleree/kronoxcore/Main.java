...
public class Main {
    ...
    static class ConfigManager {
        ...
        private void createDefaultConfig() { 
            ...
            properties.setProperty("mariaDbUrl", "jdbc:mariadb://localhost:3306/kronoxcore");
        ...
        public void setDefaultCsrfProtection(boolean defaultCsrfProtection) {
            properties.setProperty("defaultCsrfProtection", 
                                    String.valueOf(defaultCsrfProtection)); 
            saveConfig(); 
        }
        ...
        public String getMariaDbUrl() { 
            return properties.getProperty("mariaDbUrl"); 
        } 

        public void setMariaDbUrl(String mariaDbUrl) { 
            properties.setProperty("mariaDbUrl", mariaDbUrl); 
            saveConfig(); 
        } 
    ...
    static class NetworkManager { 
        ...
        private void promptForMariaDbCredentials(Scanner scanner) { 
            while (!testMariaDBConnection()) {
                System.out.print("Enter the MariaDB URL: "); 
                configManager.setMariaDbUrl(scanner.nextLine());
        ...
        public void connectToMariaDB() { 
            String url = configManager.getMariaDbUrl();
            ...
            try {
                ...
                try (Statement stmt = dbConnection.createStatement()) { 
                    String sql = "CREATE TABLE IF NOT EXISTS network_instances (" + 
                ...
                LOGGER.log(Level.SEVERE, "Failed to connect to MariaDB: " +
                        e.getMessage(), e);
            }
        }
        ...
        public void registerInstance() { 
            ...
            try (PreparedStatement stmt = dbConnection.prepareStatement(
                    "INSERT INTO network_instances(id, name, api_key, ip) VALUES (?, ?, ?, ?) " + 
                            "ON DUPLICATE KEY UPDATE api_key = ?, ip = ?")) {

                stmt.setString(1, instanceId); 
                stmt.setString(2, configManager.getInstanceName());
                stmt.setString(3, apiKey); 
                stmt.setString(4, instanceAddress);
                stmt.setString(5, apiKey); 
                stmt.setString(6, instanceAddress); 
                stmt.executeUpdate();

                LOGGER.info(String.format("KronoxCore instance registered/updated successfully with ID: %s and address: %s", 
                ...
                LOGGER.log(Level.SEVERE, "Failed to register instance in MariaDB: " +
                        e.getMessage(), e);
            }
        }
        ...
        public List<Map<String, String>> getNetworkInstances() {
            ...
            try (Statement stmt = dbConnection.createStatement();
                 ResultSet rs = stmt.executeQuery("SELECT name, ip FROM network_instances")) {

                while (rs.next()) {
                    Map<String, String> instance = new HashMap<>(); 
                    instance.put("name", rs.getString("name"));
        ...
        public void pingNetworkInstances() {
            ...
            for (Map<String, String> instance : instances) {
                if (!instance.get("name").equals(configManager.getInstanceName())) { 
                    String instanceName = instance.get("name");
                    String instanceAddress = instance.get("ip"); 
    ...
    static class LinkManager { 
            ...
            threadPool.execute(() -> { 
                ...
                try (SocketChannel targetChannel = SocketChannel.open();
                    ...
                    try {
                        ...
                        LOGGER.log(Level.SEVERE, 
                                String.format("Error closing client channel: %s",
                                                ex.getMessage()), ex);
                    } 
                } 
            });
        ...
        private void forwardData(SocketChannel inputChannel, ClientData clientData, 
            ...
            try {
                ...
                LOGGER.log(Level.WARNING, 
                        String.format("Error forwarding data from %s to target: %s",
        ...
        private void forwardData(InputStream input, SocketChannel outputChannel, 
            ...
            try { 
                ...
                LOGGER.log(Level.WARNING, 
                            String.format("Error forwarding data to client %s: %s", 
                                            clientData.clientIP, e.getMessage()), e);
                try { 
                    outputChannel.close(); 
                } catch (IOException ex) {
    ...
    static class CommandHandler {
        ...
        public String handleCommand(String input) { 
            ...
            // Using an enhanced switch statement (rule-based) 
                ...
                case "linkadd" -> { 
                    if (parts.length == 5 && 
                        ...
                        yield "Usage: linkadd <name> <targetAddress:port> " + 
                              "<mode(forward/ddosprot/filterddosprot)>"; 
                    } 
                } 
                case "linkremove" -> { 
                    if (parts.length == 2) {
